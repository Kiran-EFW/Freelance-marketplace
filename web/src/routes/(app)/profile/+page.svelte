<script lang="ts">
	import { Camera, MapPin, Phone, Mail, Globe, Bell, Award, Shield } from 'lucide-svelte';
	...
	// Mock user data
	let name = $state('Amit Verma');
	let email = $state('amit@example.com');
	let userPhone = $state('+919876543210');
	let address = $state('123, MG Road');
	let postcode = $state('560001');
	let language = $state('en');
	let role = $state<'customer' | 'provider'>('customer');
	let pointsBalance = $state(1250);
	let levelName = $state('Silver');
	let avatarUrl = $state('');
	...
	// Provider-specific
	let bio = $state('Experienced plumber with 10+ years of expertise in residential and commercial plumbing.');
	let hourlyRate = $state(500);
